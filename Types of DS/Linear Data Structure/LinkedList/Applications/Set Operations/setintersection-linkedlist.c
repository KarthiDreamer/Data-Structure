...
// Function to perform intersection on two sets
struct Node *setIntersection(struct Node *set1, struct Node *set2) {
    struct Node *result = NULL;

    struct Node *current = set1;
    while (current != NULL) {
        if (findNode(set2, current->data)) {
...
int main() {
    ...
    struct Node *result = setIntersection(set1, set2);
